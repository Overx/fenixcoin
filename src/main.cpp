...
uint256 hashGenesisBlock("0x5c048d35b4a8c139a4cede17e9037f1401f99093999539f9bf3ee6c2770c2f55");
...
const string strMessageMagic = "Fenixcoin Signed Message:\n";
...
{
    // Fenixcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // Fenixcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "fnx genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1490211739;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1489638;
	    ...
	    assert(block.hashMerkleRoot == uint256("0xbfe836a8b71f69569ca65b04c5d4e159cb8efcb22a883450ab853d0d8e5c125d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xd4, 0xc5, 0xaf, 0xeb };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FenixcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("FenixcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static FenixcoinMiner(CWallet *pwallet)
{
    printf("FenixcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Fenixcoin-miner");
    ...
    try { loop {
        ...
        printf("Running FenixcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("FenixcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FenixcoinMiner, pwallet));
}
