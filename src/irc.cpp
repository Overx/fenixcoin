...
// Copyright (c) 2011-2012 Fenixcoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("Fenixcoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #FenixcoinTEST0\r");
            Send(hSocket, "WHO #FenixcoinTEST0\r");
        } else {
            // randomly join #Fenixcoin00-#Fenixcoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #Fenixcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #Fenixcoin%02d\r", channel_number).c_str());
        }
